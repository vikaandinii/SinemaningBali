...
public class MainFrm extends javax.swing.JFrame {
    ...
    DefaultTableModel model = new DefaultTableModel(
            new String[]{"Movie Title", "Genre", "Rating", "Hour", "Minute", "Content Rating", "Ticket Price", "uri", "Poster"}, 0);
...
private void getMovies() {
    ...
    try {
        Connection conn = DBConnectClass.getConnection();
        PreparedStatement pst = conn.prepareStatement(query);
        ResultSet rs = pst.executeQuery();

        // Bersihkan model sebelum menambahkan baris baru
        model.setRowCount(0);
        ...
        // Set data dari model ke komponen UI sesuai urutan
  ...
  private ImageIcon parsePoster(byte[] posterBytes) {
    if (posterBytes != null && posterBytes.length > 0) {
        return new ImageIcon(posterBytes);
    } else {
        // Jika poster tidak tersedia, Anda dapat mengembalikan poster default atau null
        return null;
    }
}
